...
def exponential_descrease(idx_decoder, p=3):##注意力衰减函数
...
class AttentionHelper(nn.Module):
    ...
    def scalar_dot_att(self, proj_query, proj_key, proj_val, padding_mask):##计算注意力分数，输出（B,c3，l1）
        '''
...
class AttLayer(nn.Module):##完整的注意力层，输出（B,v_dim,L）
    ...
    def _block_wise_self_att(self, q,k,v, mask):##切块处理，注重局部，但边界时间步看不到上下文
        m_batchsize, c1, L = q.size()
        _,c2,L = k.size()
        _,c3,L = v.size()
        
        nb = L // self.bl##计算块数
        if L % self.bl != 0:##将L填充为b1的整数倍
    ...
    def _sliding_window_self_att(self, q,k,v, mask):##为了保证每个时间步都能看到上下文
        m_batchsize, c1, L = q.size()
        _, c2, _ = k.size()
        _, c3, _ = v.size()
        ...
        if L % self.bl != 0:
            ...
            v = torch.cat([v, torch.zeros((m_batchsize, c3, self.bl - L % self.bl)).to(device)], dim=-1)
            nb += 1
        padding_mask = torch.cat([torch.ones((m_batchsize, 1, L)).to(device) * mask[:,0:1,:], torch.zeros((m_batchsize, 1, self.bl * nb - L)).to(device)],dim=-1)##补齐填充mask
...
class MultiHeadAttLayer(nn.Module):##多层注意力模块
...
class ConvFeedForward(nn.Module):##扩张卷积前馈模块
...
class FCFeedForward(nn.Module):##全连接前馈模块，弃用，改用膨胀卷积前馈网络
...
class AttModule(nn.Module):##block架构--(B，in__channels,L)->(B,out__channels,L)
    ...
    def forward(self, x, f, mask):
        out = self.feed_forward(x)##shape不变
        out = self.alpha * self.att_layer(self.instance_norm(out), f, mask) + out##f来自decoder，根据stage的值判断value采用x1还是x2
...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        if self.channel_masking_rate > 0:##只丢弃通道这一维度
            x = x.unsqueeze(2)
            x = self.dropout(x)
            x = x.squeeze(2)
...
class Trainer:
    def __init__(self, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate):
        self.model = MyTransformer(3, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate)##3层decoder
    ...
    def train(self, save_dir, batch_gen, num_epochs, batch_size, learning_rate, batch_gen_tst=None):
        ...
        for epoch in range(num_epochs):
            ...
            while batch_gen.has_next():
                ...
                loss = 0
                for p in ps:
                    loss += self.ce(p.transpose(2, 1).contiguous().view(-1, self.num_classes), batch_target.view(-1))##计算；类别损失
                    loss += 0.15 * torch.mean(torch.clamp(
                        self.mse(F.log_softmax(p[:, :, 1:], dim=1), F.log_softmax(p.detach()[:, :, :-1], dim=1)), min=0,
                        max=16) * mask[:, :, 1:])##计算平滑性损失
            ...
            if (epoch + 1) % 10 == 0 and batch_gen_tst is not None:##每10个epoch在test测试集上测试
